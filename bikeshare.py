...
def common_station(df,station_type):
    """
    Displays the most commonly used station(s) or the most common trip, depending on the value of station_type.

    Args:
        (DataFrame) df - Pandas DataFrame containing city data
        (str) station_type - station type ('Start Station', 'End Station', 'Trip')

    Returns:
        None
    """

    # add Trip column as 'Start Station' to 'End Station'
    df = df.assign(Trip = lambda x : '«' + x['Start Station'] + '»   to   «' + x['End Station'] + '»')
    ...
    if station_type in {'Start Station','End Station'}:
